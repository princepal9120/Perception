...
async def test_chains():
    ...
    claims = await chains.extract_claims(topic="Quantum Computing", documents=test_docs)
    ...
    gaps = await chains.identify_gaps(
        topic="Quantum Computing", verified_claims=verified, iteration=1, max_iterations=3, depth=3
    )
...
async def test_graph():
    ...
    with warnings.catch_warnings():
        ...
        result = await graph.run_research(topic="Quantum Computing basics", depth=2, iterations=2)
