...
@router.post(
    "", response_model=ChatResponse, status_code=status.HTTP_201_CREATED, responses={401: {"model": ErrorResponse}}
)
async def create_chat(
    chat_data: ChatCreate, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_db)
):
    """
...
@router.get("", response_model=ChatListResponse, responses={401: {"model": ErrorResponse}})
...
async def get_chat(
    chat_id: int, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_db)
):
    """
...
async def delete_chat(
    ...
    return SuccessResponse(message="Chat deleted successfully", data={"chat_id": chat_id})
...
async def send_message(
    ...
    await service.create_message_with_docs(chat_id=chat_id, role="user", content=message_data.content)
                ...
                async with AsyncSessionLocal() as session:
                    try:
                        ...
                        if assistant_content:
                            from app.models.tables import Chat, Message

                            full_content = "".join(assistant_content)

                            message = Message(chat_id=chat_id, user_id=user_id, role="assistant", content=full_content)
                            session.add(message)
