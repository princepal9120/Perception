...
class ConversationNode(SQLModel, table=True):
    ...
    parent_id: str | None = Field(default=None, foreign_key="conversation_nodes.id", max_length=36, index=True)
    depth: int = Field(default=0, index=True)

    # Message content
    user_message: str | None = Field(default=None, sa_column=Column(Text))
    ai_message: str | None = Field(default=None, sa_column=Column(Text))

    # Metadata
    metadata_json: str | None = Field(
        default=None, sa_column=Column(Text), description="JSON metadata: tokens, model, tools used, etc."
    )

    # Branch information
    branch_name: str | None = Field(
        default=None, max_length=100, description="Human-readable branch name (e.g., 'Branch A', 'Branch A1')"
    )
    is_active: bool = Field(default=True, description="Whether this node is in the currently active branch")
    ...
    checkpoint_id: str | None = Field(
        default=None, max_length=255, index=True, description="LangGraph checkpoint ID for this node"
    )
...
class ConversationTree(SQLModel, table=True):
    ...
    # Tree metadata
    root_node_id: str = Field(foreign_key="conversation_nodes.id", max_length=36, description="ID of the root node")
    active_node_id: str = Field(
        foreign_key="conversation_nodes.id", max_length=36, description="Currently active/selected node"
    )
    ...
    __table_args__ = (Index("ix_trees_user_created", "user_id", "created_at"),)
...
class NodeRelationship(SQLModel, table=True):
    ...
    id: int | None = Field(default=None, primary_key=True)
    conversation_id: int = Field(foreign_key="chats.id", index=True)
    parent_node_id: str = Field(foreign_key="conversation_nodes.id", max_length=36, index=True)
    child_node_id: str = Field(foreign_key="conversation_nodes.id", max_length=36, index=True)
