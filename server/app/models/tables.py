...
class User(SQLModel, table=True):
    ...
    created_at: datetime = Field(
        default_factory=lambda: datetime.now(UTC), sa_column=Column(DateTime(timezone=True), server_default=func.now())
    )
...
class Chat(SQLModel, table=True):
    ...
    checkpoint_id: str | None = Field(default=None, max_length=255, index=True)

    # Branch tracking (for ChatGPT-like branching)
    parent_chat_id: int | None = Field(default=None, foreign_key="chats.id", index=True)
    branch_message_id: int | None = Field(default=None, description="Message ID this chat branched from")
