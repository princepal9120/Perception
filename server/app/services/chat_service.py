...
from app.models.tables import Chat, Document, Message, User
...
class ChatService:
    ...
    async def create_chat(self, title: str) -> Chat:
        ...
        Returns:
            Created chat
        """
        chat = Chat(user_id=self.user.id, title=title)
    ...
    async def get_user_chats(self, skip: int = 0, limit: int = 50) -> tuple[list[Chat], int]:
        ...
        # Get chats
        query = (
            select(Chat).where(Chat.user_id == self.user.id).order_by(Chat.updated_at.desc()).offset(skip).limit(limit)
        )
    ...
    async def update_chat(self, chat_id: int, title: str | None = None) -> Chat:
        """
    ...
    async def get_messages(
        self, chat_id: int, skip: int = 0, limit: int = 100, use_cache: bool = True
    ...
    async def create_message(self, chat_id: int, role: str, content: str, metadata: dict | None = None) -> Message:
        """
    ...
    async def check_rate_limit(self) -> bool:
        ...
        allowed, remaining = await redis_client.check_rate_limit(
            self.user.id, settings.RATE_LIMIT_MESSAGES_PER_MINUTE, settings.RATE_LIMIT_WINDOW_SECONDS
        )
    ...
    async def get_chat_documents(self, chat_id: int) -> list[Document]:
        ...
        query = (
            ...
            .where(Document.indexed.is_(True))
    ...
    async def create_message_with_docs(
        self, chat_id: int, role: str, content: str, metadata: dict | None = None
    ) -> Message:
        """
    ...
    async def branch_from_message(self, chat_id: int, message_id: int) -> Chat:
        ...
        logger.info(
            f"Created branch chat {new_chat.id} from chat {chat_id} message {message_id} with {len(messages_to_copy)} messages"
        )
        return new_chat
