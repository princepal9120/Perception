...
class TreeService:
    ...
    async def create_tree_for_chat(self, chat_id: int) -> ConversationTree:
        ...
        existing_tree = await self.db.execute(select(ConversationTree).where(ConversationTree.chat_id == chat_id))
        if existing_tree.scalar_one_or_none():
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Tree already exists for this chat")

        # Create root node
        root_node = ConversationNode(
            conversation_id=chat_id, parent_id=None, depth=0, user_id=self.user.id, branch_name="Main", is_active=True
        )
    ...
    async def get_or_create_tree(self, chat_id: int) -> ConversationTree:
        ...
        Returns:
            Conversation tree
        """
        result = await self.db.execute(select(ConversationTree).where(ConversationTree.chat_id == chat_id))
    ...
    async def create_node(
        ...
        if parent_id:
            ...
            relationship = NodeRelationship(
                conversation_id=chat_id, parent_node_id=parent_id, child_node_id=node.id, sibling_order=sibling_order
            )
    ...
    async def get_node(self, node_id: str) -> ConversationNode:
        ...
        """
        result = await self.db.execute(select(ConversationNode).where(ConversationNode.id == node_id))
        ...
        if node.user_id != self.user.id:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Access denied to this node")
    ...
    async def get_siblings(self, node_id: str) -> list[ConversationNode]:
        ...
        result = await self.db.execute(
            select(ConversationNode)
            .where(and_(ConversationNode.parent_id == node.parent_id, ConversationNode.id != node_id))
            .order_by(ConversationNode.created_at.asc())
        )
    ...
    async def get_tree_structure(self, chat_id: int) -> ConversationTreeStructure:
        ...
        return ConversationTreeStructure(
            tree_metadata=tree_response, nodes=node_data_list, adjacency_list=adjacency_list
        )

    # ==================== Branching Operations ====================

    async def fork_node(self, node_id: str, branch_name: str | None = None) -> ConversationNode:
        """
    ...
    async def set_active_node(self, chat_id: int, node_id: str) -> ConversationTree:
        ...
        if node.conversation_id != chat_id:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Node does not belong to this chat")

        # Update all nodes to inactive
        await self.db.execute(select(ConversationNode).where(ConversationNode.conversation_id == chat_id))
    ...
    async def get_tree_statistics(self, chat_id: int) -> TreeStatistics:
        ...
        result = await self.db.execute(
            select(
                ...
                func.sum(func.case((ConversationNode.user_message.isnot(None), 1), else_=0)).label("user_messages"),
                func.sum(func.case((ConversationNode.ai_message.isnot(None), 1), else_=0)).label("ai_messages"),
            ).where(ConversationNode.conversation_id == chat_id)
        )
        stats = result.one()
        ...
        all_nodes_result = await self.db.execute(
            select(ConversationNode.id).where(ConversationNode.conversation_id == chat_id)
        )
        all_node_ids = {row[0] for row in all_nodes_result.all()}

        parent_nodes_result = await self.db.execute(
            select(func.distinct(NodeRelationship.parent_node_id)).where(NodeRelationship.conversation_id == chat_id)
        )
    ...
    async def migrate_linear_chat_to_tree(self, chat_id: int) -> tuple[ConversationTree, int]:
        ...
        for i in range(0, len(messages), 2):
            ...
            logger.info(
                f"Processing pair {i//2 + 1}: user={bool(user_msg)} ({user_msg.content[:30] if user_msg else 'None'}...), ai={bool(ai_msg)} ({ai_msg.content[:30] if ai_msg else 'None'}...)"
            )
            ...
            logger.info(
                f"Created node {node.id} with depth={node.depth}, user_msg_len={len(user_msg.content) if user_msg else 0}, ai_msg_len={len(ai_msg.content) if ai_msg else 0}"
            )
    ...
    async def _count_children(self, node_id: str) -> int:
        ...
        result = await self.db.execute(
            select(func.count(ConversationNode.id)).where(ConversationNode.parent_id == node_id)
        )
    ...
    async def _count_siblings(self, parent_id: str) -> int:
        ...
        result = await self.db.execute(
            select(func.count(ConversationNode.id)).where(ConversationNode.parent_id == parent_id)
        )
        return result.scalar() or 0
