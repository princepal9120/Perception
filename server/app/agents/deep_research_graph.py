...
class DeepResearchGraph:
    ...
    def __init__(
        self,
        llm: Any | None = None,
        runtime_provider_config: RuntimeProviderConfig | None = None,
    ):
        """
    ...
    def _build_graph(self) -> StateGraph:
        ...
        graph_builder.add_conditional_edges(
            ...
            {"continue": "retriever_node", "finish": "final_synthesis_node"},
        )
    ...
    async def extract_claims_node(self, state: DeepResearchState) -> DeepResearchState:
        ...
        claims = await self.chains.extract_claims(topic=state["topic"], documents=state["current_documents"])
    ...
    async def verify_claims_node(self, state: DeepResearchState) -> DeepResearchState:
        ...
        verified = await self.chains.verify_claims(
            claims=state.get("current_claims", []), documents=state["current_documents"]
        )
    ...
    def _generate_initial_queries(self, topic: str, depth: int) -> list[str]:
        ...
        """
        base_queries = [f"{topic} overview", f"{topic} latest research", f"{topic} technical details"]

        if depth >= 3:
            base_queries.extend([f"{topic} academic papers", f"{topic} case studies"])

        if depth >= 4:
            base_queries.extend([f"{topic} arxiv papers", f"{topic} research methodology"])

        if depth == 5:
            base_queries.extend([f"{topic} cutting edge research", f"{topic} future directions"])
    ...
    async def run_research(self, topic: str, depth: int = 3, iterations: int = 3) -> dict[str, Any]:
        """
        Run deep research.
