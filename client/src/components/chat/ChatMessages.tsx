...
export const ChatMessages = () => {
  ...
  const { conversations, currentConversationId, isStreaming, streamingMessage, streamingSearchInfo } = useChatStore();
  ...
  const handleSuggestedPrompt = async (prompt: string, deepResearch?: boolean) => {
    // TODO: Implement deep research mode flag in the API call
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-3 sm:px-4 md:px-6 py-4 sm:py-6 md:py-8 space-y-4 sm:space-y-6">
      {messages.length === 0 && !isStreaming ? (
        ...
        >
          {/* Avatar */}
          <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center flex-shrink-0 ${message.role === "assistant"
            ? "gradient-primary"
            : "bg-muted"
            }`}>
          ...
          {/* Message Content */}
          <div className={`flex-1 max-w-full sm:max-w-[85%] md:max-w-2xl ${message.role === "user" ? "flex justify-end" : ""}`}>
            <div className={`inline-block p-3 sm:p-4 rounded-xl sm:rounded-2xl ${message.role === "assistant"
              ? "bg-card border border-border shadow-sm dark:shadow-none text-left w-full"
              : "bg-primary text-primary-foreground"
              }`}>
              {message.role === "assistant" ? (
                <>
                  {message.searchInfo && <DeepResearchDisplay searchInfo={message.searchInfo} isStreaming={false} />}
        ...
        >
          ...
          <div className="flex-1 max-w-full sm:max-w-[85%] md:max-w-2xl">
            <div className="inline-block p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-card border border-border shadow-sm dark:shadow-none w-full">
              {streamingSearchInfo && <DeepResearchDisplay searchInfo={streamingSearchInfo} isStreaming={true} />}
